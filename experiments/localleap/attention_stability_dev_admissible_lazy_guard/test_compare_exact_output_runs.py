...
def test_loader_rejects_duplicate_ids_and_missing_fields():
    with tempfile.TemporaryDirectory() as directory:
        ...
        try:
            ...
            assert "duplicate stable identity" in str(error)
...
def test_gsm_task_schema_uses_stable_id_and_target_hash():
    gsm = record()
    gsm.pop("task_id")
    gsm.pop("normalized_gold")
    gsm["stable_task_id"] = "index_0"
    gsm["target_hash"] = "target-hash"
    parent = {"index_0": gsm}
    candidate_record = copy.deepcopy(gsm)
    candidate_record["nfe"] = 80
    summary = compare_records(parent, {"index_0": candidate_record})
    assert summary["all_invariants_pass"]
    assert summary["nfe_reduction"] == 20
...
if __name__ == "__main__":
    ...
    test_gsm_task_schema_uses_stable_id_and_target_hash()
    print("5 exact-output evaluator tests passed")
