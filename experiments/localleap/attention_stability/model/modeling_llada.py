...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        # Optional read-only dependency probe used by the attention-stability
        # decoder.  It deliberately does not change the normal SDPA/FlashAttention
        # path below.  Only the requested query rows are materialized, and only
        # their probabilities on the requested key positions are retained.
        dependency_tracker = getattr(self, "_dependency_tracker", None)
        if dependency_tracker is not None:
            positions = dependency_tracker["positions"]
            q_probe = q.index_select(-2, positions).float()
            k_probe = k
            if q_probe.size(1) != k_probe.size(1):
                assert q_probe.size(1) % k_probe.size(1) == 0
                k_probe = k_probe.repeat_interleave(q_probe.size(1) // k_probe.size(1), dim=1)
            scores = torch.matmul(q_probe, k_probe.float().transpose(-2, -1)) / math.sqrt(q_probe.size(-1))
            if attention_bias is not None:
                scores = scores + attention_bias.index_select(-2, positions).float()
            probabilities = torch.softmax(scores, dim=-1)
            local_attention = probabilities.index_select(-1, positions).mean(dim=1)
            if dependency_tracker["sum"] is None:
                dependency_tracker["sum"] = local_attention
            else:
                dependency_tracker["sum"] += local_attention
            dependency_tracker["count"] += 1
