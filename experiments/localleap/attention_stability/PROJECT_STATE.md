...
The next method under test is Response-Credit Draft Exchange. It is an original,
small descendant of the tau-0.004 symmetric parent rather than a replacement
baseline. Its vertical signal increments only when a candidate survives an
actual strong-dependency conditioning event, resets when that event changes the
top-1, and does not mistake weak/no-new-information steps for extra evidence.
Its horizontal signal generates a parent-ordered anchor and a response-credit
explorer, preserves their full-draft agreement as a skeleton, masks every
disagreement, and re-denoises those positions with the same symmetric exclusion.
For code only, a deterministic selector may use prompt-visible examples and
type-derived behavioral consensus; it is explicitly barred from reference
solutions and hidden tests. A fixed-budget response-credit child retains the
original parallel commit budget for speed comparisons.

Fourteen selector tests, five differential-selector tests and two draft-exchange
tests pass. The versioned queue `response_credit_exchange_20260717_v1` is staged
to start only after `best_symmetric_long_20260716_v2` exits. It uses original
LLaDA as the formal baseline, runs real-model 2-example preflights, then 32/64
sample gates before any full HumanEval/MBPP promotion. MATH-500 and GSM8K use no
execution selector and directly test shared-skeleton repair. Every promotion is
