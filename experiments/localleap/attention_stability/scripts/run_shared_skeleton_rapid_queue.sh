...
queue_id=${ATTENTION_QUEUE_ID:-shared_skeleton_rapid_20260719_v1}
llada_root=${LLADA_ROOT:-/root/autodl-tmp/LocalLeap/llada_slot_shared_skeleton}
source_root=${ATTENTION_SOURCE_ROOT:-/root/autodl-tmp/dlm-seq-flow/experiments/localleap/attention_stability_dev_shared_skeleton}
runner=${source_root}/scripts/run_best_symmetric_benchmark.sh
controller=${source_root}/scripts/run_shared_skeleton_rapid_queue.sh
...
if [[ ! -s "${frozen}" ]]; then
  sha256sum generate.py eval_llada.py compare_paired_task_runs.py audit_attention_stability.py \
    ...
    "${source_root}/shared_skeleton_preregistration_20260719.json" >"${frozen}"
fi
verify
...
echo "method=trajectory_shared_skeleton_tau_0.004"
echo "selection=one_common_context_forward_over_final_parent_disagreements"
...
run_gpu smoke_sscv_n4 0 "${runner}" humaneval 0 128 trajectory_shared_skeleton 0.004 trace smoke_sscv_n4 4 256 & p0=$!
...
require_done "${run_base}/humaneval/smoke_sscv_n4" "${run_base}/humaneval/smoke_fast_n4"
pair_runs smoke_sscv_vs_fast "${run_base}/humaneval/smoke_fast_n4" "${run_base}/humaneval/smoke_sscv_n4"

# Fast labeled discovery gate on the long-standing HumanEval/0-31 dev slice.
run_gpu he_sscv_dev32 0 "${runner}" humaneval 0 128 trajectory_shared_skeleton 0.004 trace he_sscv_dev32 32 256 & p0=$!
(
...
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/humaneval/he_sscv_dev32" "${run_base}/humaneval/he_fast_dev32" "${run_base}/humaneval/he_base_dev32"
pair_runs he_sscv_vs_fast_dev32 "${run_base}/humaneval/he_fast_dev32" "${run_base}/humaneval/he_sscv_dev32"
pair_runs he_sscv_vs_base_dev32 "${run_base}/humaneval/he_base_dev32" "${run_base}/humaneval/he_sscv_dev32"
dfast="${queue_root}/paired/he_sscv_vs_fast_dev32/paired_summary.json"
dbase="${queue_root}/paired/he_sscv_vs_base_dev32/paired_summary.json"
...
run_gpu he_sscv_full164 0 "${runner}" humaneval 0 128 trajectory_shared_skeleton 0.004 trace he_sscv_full164 164 256 & p0=$!
(
...
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/humaneval/he_sscv_full164" "${run_base}/humaneval/he_fast_full164" "${run_base}/humaneval/he_base_full164"
pair_runs he_sscv_vs_fast_full164 "${run_base}/humaneval/he_fast_full164" "${run_base}/humaneval/he_sscv_full164"
pair_runs he_sscv_vs_base_full164 "${run_base}/humaneval/he_base_full164" "${run_base}/humaneval/he_sscv_full164"
for name in sscv fast base; do
...
run_stage he_holdout_vs_fast /root/miniconda3/bin/python "${llada_root}/compare_paired_task_runs.py" \
  "${queue_root}/holdout/he_fast_96_164.jsonl" "${queue_root}/holdout/he_sscv_96_164.jsonl" \
  --baseline-config "${run_base}/humaneval/he_fast_full164/run_config.txt" \
  --method-config "${run_base}/humaneval/he_sscv_full164/run_config.txt" \
  --method-log "${queue_root}/he_sscv_full164.log" --output-dir "${queue_root}/paired/he_holdout_vs_fast"
run_stage he_holdout_vs_base /root/miniconda3/bin/python "${llada_root}/compare_paired_task_runs.py" \
  "${queue_root}/holdout/he_base_96_164.jsonl" "${queue_root}/holdout/he_sscv_96_164.jsonl" \
  --baseline-config "${run_base}/humaneval/he_base_full164/run_config.txt" \
  --method-config "${run_base}/humaneval/he_sscv_full164/run_config.txt" \
  --method-log "${queue_root}/he_sscv_full164.log" --output-dir "${queue_root}/paired/he_holdout_vs_base"
...
run_gpu math_sscv_n50 0 "${runner}" localleap_math500 0 128 trajectory_shared_skeleton 0.004 trace math_sscv_n50 50 256 & p0=$!
(
...
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/localleap_math500/math_sscv_n50" "${run_base}/localleap_math500/math_fast_n50" "${run_base}/localleap_math500/math_base_n50"
pair_runs math_sscv_vs_fast_n50 "${run_base}/localleap_math500/math_fast_n50" "${run_base}/localleap_math500/math_sscv_n50"
pair_runs math_sscv_vs_base_n50 "${run_base}/localleap_math500/math_base_n50" "${run_base}/localleap_math500/math_sscv_n50"
mf="${queue_root}/paired/math_sscv_vs_fast_n50/paired_summary.json"; mb="${queue_root}/paired/math_sscv_vs_base_n50/paired_summary.json"
...
(
  run_gpu mbpp_sscv_n50 0 "${runner}" mbpp 0 128 trajectory_shared_skeleton 0.004 trace mbpp_sscv_n50 50 256
  ...
  pair_runs mbpp_sscv_vs_fast_n50 "${run_base}/mbpp/mbpp_fast_n50" "${run_base}/mbpp/mbpp_sscv_n50"
  pair_runs mbpp_sscv_vs_base_n50 "${run_base}/mbpp/mbpp_base_n50" "${run_base}/mbpp/mbpp_sscv_n50"
) & p0=$!
(
  run_gpu gsm_sscv_n64 1 "${runner}" gsm8k 0 128 trajectory_shared_skeleton 0.004 trace gsm_sscv_n64 64 256
  ...
  pair_runs gsm_sscv_vs_fast_n64 "${run_base}/gsm8k/gsm_fast_n64" "${run_base}/gsm8k/gsm_sscv_n64"
  pair_runs gsm_sscv_vs_base_n64 "${run_base}/gsm8k/gsm_base_n64" "${run_base}/gsm8k/gsm_sscv_n64"
) & p1=$!
wait "${p0}" || true; wait "${p1}" || true
