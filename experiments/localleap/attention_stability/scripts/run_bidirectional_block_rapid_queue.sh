...
queue_id=${ATTENTION_QUEUE_ID:-bidirectional_block_rapid_20260719_v1}
llada_root=${LLADA_ROOT:-/root/autodl-tmp/LocalLeap/llada_slot_bidirectional_block}
source_root=${ATTENTION_SOURCE_ROOT:-/root/autodl-tmp/dlm-seq-flow/experiments/localleap/attention_stability_dev_bidirectional_block}
runner=${source_root}/scripts/run_best_symmetric_benchmark.sh
controller=${source_root}/scripts/run_bidirectional_block_rapid_queue.sh
...
if [[ ! -s "${frozen}" ]]; then
  sha256sum generate.py eval_llada.py compare_paired_task_runs.py audit_attention_stability.py \
    ...
    "${source_root}/bidirectional_block_preregistration_20260719.json" >"${frozen}"
fi
verify
...
echo "method=trajectory_bidirectional_block_tau_0.004"
echo "selection=bidirectional_block_full_draft_verification"
...
run_gpu smoke_bbv_n4 0 "${runner}" humaneval 0 128 trajectory_bidirectional_block 0.004 trace smoke_bbv_n4 4 256 & p0=$!
...
require_done "${run_base}/humaneval/smoke_bbv_n4" "${run_base}/humaneval/smoke_fast_n4"
pair_runs smoke_bbv_vs_fast "${run_base}/humaneval/smoke_fast_n4" "${run_base}/humaneval/smoke_bbv_n4"

# Ten-example fail-fast gate on the exact slice that exposed v7's dense-mask
# failure.  This costs only a few minutes and prevents another weak 32 run.
run_gpu he_bbv_dev10 0 "${runner}" humaneval 0 128 trajectory_bidirectional_block 0.004 trace he_bbv_dev10 10 256 & p0=$!
(
  run_gpu he_fast_dev10 1 "${runner}" humaneval 0 128 symmetric_fast 0.004 trace he_fast_dev10 10 256
  run_gpu he_base_dev10 1 "${runner}" humaneval 0 128 baseline 0 trace he_base_dev10 10 256
) & p1=$!
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/humaneval/he_bbv_dev10" "${run_base}/humaneval/he_fast_dev10" "${run_base}/humaneval/he_base_dev10"
pair_runs he_bbv_vs_fast_dev10 "${run_base}/humaneval/he_fast_dev10" "${run_base}/humaneval/he_bbv_dev10"
pair_runs he_bbv_vs_base_dev10 "${run_base}/humaneval/he_base_dev10" "${run_base}/humaneval/he_bbv_dev10"
tfast="${queue_root}/paired/he_bbv_vs_fast_dev10/paired_summary.json"
tbase="${queue_root}/paired/he_bbv_vs_base_dev10/paired_summary.json"
tm=$(field "${tfast}" method_correct); tfastn=$(field "${tfast}" baseline_correct); tbasen=$(field "${tbase}" baseline_correct); tr=$(field "${tfast}" method_only)
echo "dev10_gate method=${tm} fast=${tfastn} base=${tbasen} recoveries=${tr}"
if ! (( tm > tfastn && tm >= tbasen && tr > 0 )); then
  touch "${queue_root}/DONE_REJECTED_DEV10" "${queue_root}/DONE"
  exit 0
fi

# Fast labeled discovery gate on the long-standing HumanEval/0-31 dev slice.
run_gpu he_bbv_dev32 0 "${runner}" humaneval 0 128 trajectory_bidirectional_block 0.004 trace he_bbv_dev32 32 256 & p0=$!
(
...
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/humaneval/he_bbv_dev32" "${run_base}/humaneval/he_fast_dev32" "${run_base}/humaneval/he_base_dev32"
pair_runs he_bbv_vs_fast_dev32 "${run_base}/humaneval/he_fast_dev32" "${run_base}/humaneval/he_bbv_dev32"
pair_runs he_bbv_vs_base_dev32 "${run_base}/humaneval/he_base_dev32" "${run_base}/humaneval/he_bbv_dev32"
dfast="${queue_root}/paired/he_bbv_vs_fast_dev32/paired_summary.json"
dbase="${queue_root}/paired/he_bbv_vs_base_dev32/paired_summary.json"
...
run_gpu he_bbv_full164 0 "${runner}" humaneval 0 128 trajectory_bidirectional_block 0.004 trace he_bbv_full164 164 256 & p0=$!
(
...
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/humaneval/he_bbv_full164" "${run_base}/humaneval/he_fast_full164" "${run_base}/humaneval/he_base_full164"
pair_runs he_bbv_vs_fast_full164 "${run_base}/humaneval/he_fast_full164" "${run_base}/humaneval/he_bbv_full164"
pair_runs he_bbv_vs_base_full164 "${run_base}/humaneval/he_base_full164" "${run_base}/humaneval/he_bbv_full164"
for name in bbv fast base; do
...
run_stage he_holdout_vs_fast /root/miniconda3/bin/python "${llada_root}/compare_paired_task_runs.py" \
  "${queue_root}/holdout/he_fast_96_164.jsonl" "${queue_root}/holdout/he_bbv_96_164.jsonl" \
  --baseline-config "${run_base}/humaneval/he_fast_full164/run_config.txt" \
  --method-config "${run_base}/humaneval/he_bbv_full164/run_config.txt" \
  --method-log "${queue_root}/he_bbv_full164.log" --output-dir "${queue_root}/paired/he_holdout_vs_fast"
run_stage he_holdout_vs_base /root/miniconda3/bin/python "${llada_root}/compare_paired_task_runs.py" \
  "${queue_root}/holdout/he_base_96_164.jsonl" "${queue_root}/holdout/he_bbv_96_164.jsonl" \
  --baseline-config "${run_base}/humaneval/he_base_full164/run_config.txt" \
  --method-config "${run_base}/humaneval/he_bbv_full164/run_config.txt" \
  --method-log "${queue_root}/he_bbv_full164.log" --output-dir "${queue_root}/paired/he_holdout_vs_base"
...
run_gpu math_bbv_n50 0 "${runner}" localleap_math500 0 128 trajectory_bidirectional_block 0.004 trace math_bbv_n50 50 256 & p0=$!
(
...
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/localleap_math500/math_bbv_n50" "${run_base}/localleap_math500/math_fast_n50" "${run_base}/localleap_math500/math_base_n50"
pair_runs math_bbv_vs_fast_n50 "${run_base}/localleap_math500/math_fast_n50" "${run_base}/localleap_math500/math_bbv_n50"
pair_runs math_bbv_vs_base_n50 "${run_base}/localleap_math500/math_base_n50" "${run_base}/localleap_math500/math_bbv_n50"
mf="${queue_root}/paired/math_bbv_vs_fast_n50/paired_summary.json"; mb="${queue_root}/paired/math_bbv_vs_base_n50/paired_summary.json"
...
(
  run_gpu mbpp_bbv_n50 0 "${runner}" mbpp 0 128 trajectory_bidirectional_block 0.004 trace mbpp_bbv_n50 50 256
  ...
  pair_runs mbpp_bbv_vs_fast_n50 "${run_base}/mbpp/mbpp_fast_n50" "${run_base}/mbpp/mbpp_bbv_n50"
  pair_runs mbpp_bbv_vs_base_n50 "${run_base}/mbpp/mbpp_base_n50" "${run_base}/mbpp/mbpp_bbv_n50"
) & p0=$!
(
  run_gpu gsm_bbv_n64 1 "${runner}" gsm8k 0 128 trajectory_bidirectional_block 0.004 trace gsm_bbv_n64 64 256
  ...
  pair_runs gsm_bbv_vs_fast_n64 "${run_base}/gsm8k/gsm_fast_n64" "${run_base}/gsm8k/gsm_bbv_n64"
  pair_runs gsm_bbv_vs_base_n64 "${run_base}/gsm8k/gsm_base_n64" "${run_base}/gsm8k/gsm_bbv_n64"
) & p1=$!
wait "${p0}" || true; wait "${p1}" || true
