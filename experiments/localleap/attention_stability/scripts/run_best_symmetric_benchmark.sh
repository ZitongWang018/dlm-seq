...
case "${profile}" in
  ...
  trajectory_coverage_consensus_block)
    ...
    ;;
  trajectory_convergent_coverage_consensus_block)
    # Strong likelihood gains must reduce conditioned invalidations; otherwise
  ...
  trajectory_confirmed_public_guard)
    ...
    ;;
  trajectory_lazy_confirmed_public_guard)
    # Exact-output v11 acceleration.  Skip original LLaDA when the confirmed
    # parent already exhausts all prompt-visible checks; otherwise run the
  ...
  trajectory_early_lazy_confirmed_public_guard)
    ...
    ;;
  trajectory_localized_evidence_conflict_repair)
    ...
    # re-denoise only disagreements in the strongest opposing block, and
    # retain the branch only when the same verifier prefers it. Prompt-visible
    # checks are non-regressive.
    [[ "${steps}" == "128" && "${block_length}" == "32" ]] || {
