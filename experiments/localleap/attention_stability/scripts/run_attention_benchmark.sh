...
{
  echo "schema=attention_recovery_run_v1"
...
if [[ "${task}" == "humaneval" ]]; then
  ...
  if [[ "${profile}" != "baseline" ]]; then
    ...
    if [[ "${limit}" == "full" ]]; then
      baseline_samples=${llada_root}/results/baseline/humaneval_len256_blen32_0shot/__root__autodl-tmp__model__LLaDA__instruct/samples_humaneval_2026-07-13T15-29-41.470324.jsonl
  ...
  /root/miniconda3/bin/python audit_lm_eval_task.py "${samples}" "${results_json}" \
    ...
    --expected-records "${expected_records}" --output-dir "${audit_dir}"
fi
