...
llada_root=${LLADA_ROOT:-/root/autodl-tmp/LocalLeap/llada_slot_sparse_context_repair_v19}
...
fair_queue_id=${FAIR_QUEUE_ID:-fair_three_arm_leakage_recovery_20260720_v2}
queue_id=${ATTENTION_QUEUE_ID:-sparse_context_repair_rapid_20260720_v1}
queue_root=${llada_root}/results/experiment_queues/${queue_id}
fair_queue=${parent_root}/results/experiment_queues/${fair_queue_id}
v18_queue=${v18_root}/results/experiment_queues/${v18_queue_id}
bootstrap=${llada_root}/scripts/run_sparse_context_repair_after_fair.sh
...
echo "waiting_for_fair_queue=${fair_queue}"
wait_terminal "${fair_queue}"
[[ -e "${fair_queue}/DONE" && ! -e "${fair_queue}/FAILED" ]] || {
  touch "${queue_root}/BLOCKED_FAIR_FAILED"; exit 22; }

echo "waiting_for_v18_queue=${v18_queue}"
wait_terminal "${v18_queue}"
[[ -e "${v18_queue}/DONE" && ! -e "${v18_queue}/FAILED" ]] || {
  touch "${queue_root}/BLOCKED_V18_INFRASTRUCTURE_FAILURE"; exit 23; }
...
[[ -e "${v18_queue}/REJECTED" ]] || {
  touch "${queue_root}/BLOCKED_V18_TERMINAL_WITHOUT_DECISION"; exit 24; }

verify_bootstrap
echo "launching_sparse_context_repair_after_fair"
