...
fair_queue=${llada_root}/results/experiment_queues/${fair_id}
recovery_id=${RECOVERY_ID:-full4_leakage_recovery_20260719_v2}
queue_root=${llada_root}/results/experiment_queues/${recovery_id}
controller=${canonical_root}/scripts/run_full4_leakage_recovery_and_resume.sh
...
resume_downstream() {
  ...
  if [[ ! -e "${fair_queue}/DONE" && ! -e "${fair_queue}/FAILED" ]]; then
    nohup env CANONICAL_ROOT="${canonical_root}" V15_ROOT="${llada_root}" V18_ROOT="${v18_root}" \
      ATTENTION_QUEUE_ID="${fair_id}" bash "${canonical_root}/scripts/run_fair_three_arm_reproduction_queue.sh" \
      >"${fair_queue}/launcher_recovery_v2.log" 2>&1 &
    printf '%s\n' "$!" >"${fair_queue}/launcher_recovery_v2.pid"
  fi
  touch "${queue_root}/DOWNSTREAM_RESUMED"
}
