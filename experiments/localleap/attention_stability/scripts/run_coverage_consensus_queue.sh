...
queue_id=${ATTENTION_QUEUE_ID:-trajectory_coverage_consensus_20260719_v1}
llada_root=${LLADA_ROOT:-/root/autodl-tmp/LocalLeap/llada_slot_coverage_consensus}
source_root=${ATTENTION_SOURCE_ROOT:-/root/autodl-tmp/dlm-seq-flow/experiments/localleap/attention_stability_dev_coverage_consensus}
runner=${source_root}/scripts/run_best_symmetric_benchmark.sh
controller=${source_root}/scripts/run_coverage_consensus_queue.sh
...
if [[ ! -s "${frozen}" ]]; then
  sha256sum generate.py eval_llada.py compare_paired_task_runs.py audit_attention_stability.py \
    ...
    "${source_root}/coverage_consensus_preregistration_20260719.json" >"${frozen}"
fi
verify
...
echo "method=trajectory_coverage_consensus_block_tau_0.004"
echo "selection=one_nat_or_one_revision_per_disagreement_plus_original_schedule_consensus"
...
# Fresh comparators under the coverage-consensus source make 64..95 formal.
# formal; the earlier comparator runs are not reused for the final claim.
run_gpu he_coverage_n96 0 "${runner}" humaneval 0 128 trajectory_coverage_consensus_block 0.004 trace he_coverage_n96 96 256 & p0=$!
(
...
require_done "${run_base}/humaneval/he_coverage_n96" "${run_base}/humaneval/he_fast_n96" "${run_base}/humaneval/he_base_n96"
pair_runs he_coverage_vs_fast_n96 "${run_base}/humaneval/he_fast_n96" "${run_base}/humaneval/he_coverage_n96"
pair_runs he_coverage_vs_base_n96 "${run_base}/humaneval/he_base_n96" "${run_base}/humaneval/he_coverage_n96"
for name in fast base coverage; do
...
run_stage he_holdout_vs_fast /root/miniconda3/bin/python "${llada_root}/compare_paired_task_runs.py" \
  "${queue_root}/holdout/he_fast_64_96.jsonl" "${queue_root}/holdout/he_coverage_64_96.jsonl" \
  --baseline-config "${run_base}/humaneval/he_fast_n96/run_config.txt" \
  --method-config "${run_base}/humaneval/he_coverage_n96/run_config.txt" \
  --method-log "${queue_root}/he_coverage_n96.log" --output-dir "${queue_root}/paired/he_holdout_vs_fast"
run_stage he_holdout_vs_base /root/miniconda3/bin/python "${llada_root}/compare_paired_task_runs.py" \
  "${queue_root}/holdout/he_base_64_96.jsonl" "${queue_root}/holdout/he_coverage_64_96.jsonl" \
  --baseline-config "${run_base}/humaneval/he_base_n96/run_config.txt" \
  --method-config "${run_base}/humaneval/he_coverage_n96/run_config.txt" \
  --method-log "${queue_root}/he_coverage_n96.log" --output-dir "${queue_root}/paired/he_holdout_vs_base"
...
(
  run_gpu math_coverage_n50 0 "${runner}" localleap_math500 0 128 trajectory_coverage_consensus_block 0.004 trace math_coverage_n50 50 256
...
wait "${p0}" || true; wait "${p1}" || true
require_done "${run_base}/localleap_math500/math_coverage_n50" "${run_base}/localleap_math500/math_fast_n50" "${run_base}/localleap_math500/math_base_n50"
pair_runs math_coverage_vs_fast_n50 "${run_base}/localleap_math500/math_fast_n50" "${run_base}/localleap_math500/math_coverage_n50"
pair_runs math_coverage_vs_base_n50 "${run_base}/localleap_math500/math_base_n50" "${run_base}/localleap_math500/math_coverage_n50"
mf="${queue_root}/paired/math_coverage_vs_fast_n50/paired_summary.json"; mb="${queue_root}/paired/math_coverage_vs_base_n50/paired_summary.json"
...
(
  ...
  run_gpu mbpp_coverage_n50 0 "${runner}" mbpp 0 128 trajectory_coverage_consensus_block 0.004 trace mbpp_coverage_n50 50 256
  pair_runs mbpp_coverage_vs_base_n50 "${run_base}/mbpp/mbpp_base_n50" "${run_base}/mbpp/mbpp_coverage_n50"
  pair_runs mbpp_coverage_vs_fast_n50 "${run_base}/mbpp/mbpp_fast_n50" "${run_base}/mbpp/mbpp_coverage_n50"
) & p0=$!
(
  ...
  run_gpu gsm_coverage_n64 1 "${runner}" gsm8k 0 128 trajectory_coverage_consensus_block 0.004 trace gsm_coverage_n64 64 256
  pair_runs gsm_coverage_vs_base_n64 "${run_base}/gsm8k/gsm_base_n64" "${run_base}/gsm8k/gsm_coverage_n64"
  pair_runs gsm_coverage_vs_fast_n64 "${run_base}/gsm8k/gsm_fast_n64" "${run_base}/gsm8k/gsm_coverage_n64"
) & p1=$!
wait "${p0}" || true; wait "${p1}" || true
