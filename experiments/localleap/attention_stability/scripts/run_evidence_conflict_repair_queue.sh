...
llada_root=${LLADA_ROOT:-/root/autodl-tmp/LocalLeap/llada_slot_evidence_conflict_repair}
...
queue_id=${ATTENTION_QUEUE_ID:-evidence_conflict_repair_20260719_v1}
...
controller=${llada_root}/scripts/run_evidence_conflict_repair_queue.sh
...
if [[ ! -s "${frozen}" ]]; then
  sha256sum generate.py eval_llada.py differential_selector.py \
    ...
    evidence_conflict_repair_preregistration_20260719.json \
...
run_stage leakage_static /root/miniconda3/bin/python audit_generation_leakage.py \
  --source-root "${llada_root}" --expected-profile trajectory_evidence_conflict_repair \
...
profile=trajectory_evidence_conflict_repair
...
summary={
    "schema":"evidence_conflict_repair_full4_v1",
...
echo "evidence_conflict_repair_queue_complete"
